...
% \title{Introduction to Non-Transferable Token (NTT)}
% \author{Neruthes}
\date{2022-04-29}
...
\Nmaketitle{Introduction to Non-Transferable Token (NTT)}
...
A NTT is a Signed Token Grant Certificate (STGC) granted to a Certified Token Owner (CTO) by an Token Issuing Authority (TIA),
...
\begin{compactitem}
	...
	\begin{compactitem}
		...
		\item DNSSEC-guarded DNS TXT record
	\end{compactitem}
...
The TIA shall write a Authority Keypair Declaration (AKD) which looks like:
...
\textbf{HTTPS}: The file should be available at \icode{https://example.com/example.com.sakd}.
